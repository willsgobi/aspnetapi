...
namespace DevIO.Api.Extensions
{
    public class IdentityMensagensPortugues : IdentityErrorDescriber
    {
        ...
        {
            return new IdentityError { Code = nameof(ConcurrencyFailure), Description = "Falga de concorrência otimista, o objeto foi modificado." };
        }
        ...
        {
            return new IdentityError { Code = nameof(InvalidEmail), Description = $"Email {email} já está sendo utilizado." };
        }
        ...
        {
            return new IdentityError { Code = nameof(DuplicateRoleName), Description = $"A permissão '{role}' já está semdo utilizada." };
        }

        public override IdentityError UserAlreadyHasPassword()
        {
            return new IdentityError { Code = nameof(UserAlreadyHasPassword), Description = $"Usuário já possui uma senha defenida." };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordRequiresDigit), Description = "Senhas devem conter ao menos um dígito ('0' - '9')." };
        }
